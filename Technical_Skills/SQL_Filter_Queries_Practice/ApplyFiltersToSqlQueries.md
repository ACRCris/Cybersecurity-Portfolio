...
SQL is used to study a possible security incident that occurred outside of working hours. The tables `employees` and `login_attempts` are analyzed. The`login_attempts` table contains information about login attempts, including the username, date and time of the attempt, whether it was successful, the country and the IP address used. This information is useful to determine if there were unauthorized access attempts or if employees were working outside of their regular hours. 

By another hand, the `employees` table contains information about employees, including their employee ID, device ID, username, department, and office location. This information is useful to identify which employees in which departments need to update their devices.

## Retrieve after hours failed login attempts

The following query retrieves all failed login attempts that occurred outside of working hours (after 6:00 PM).
...
- The `SELECT` is used to indicate that we want to retrieve some columns from the `log_in_attempts` table.
- The `*` tells all columns are used in the before command `SELECT`. 
- The `FROM` clause specifies the table from which we want to retrieve the data, in this case, `log_in_attempts`.
- The `WHERE` clause is used to filter the results based on specific conditions. In this case, we are looking for login attempts that occurred after 6:00 PM (`login_time > '18:00'`) and were unsuccessful (`success = False`).
- The `AND` operator is used to combine the two conditions, ensuring that only login attempts that meet both criteria are returned.

The result shows that 19 login attempts were made by different users, countries, and IP addresses after working hours, indicating potential unauthorized access attempts.

## Retrieve login attempts on specific dates

Then we use the following query to retrieve all login attempts that occurred between 08/05/2022 and 09/05/2022:
...
```
The first part of the query `SELECT * FROM login_attempts WHERE` is the same as the previous and works in the same way. The difference is how we filter the results. In this case, we the field `login_date` is used to filter the results based on specific dates. The `OR` operator is used to combine the two conditions, ensuring that login attempts that occurred on either date are returned.

The result shows that there were 75 login attempts made by different users, and there were various unsuccessful attempts on both dates in working hours and outside of working hours.

## Retrieve login attempts outside of Mexico

We want to retrieve all login attempts that did not occur in Mexico. The following query is used:
...
+----------+----------+------------+------------+---------+-----------------+---------+
144 rows in set (0.036 sec)
```

The new in the query are the `NOT` operator, the `LIKE` clause and the pattern `%`. The `NOT` operator negates the condition, meaning we want to exclude records that match the specified pattern. The `LIKE` clause is used to match the `country` field against a pattern `'MEX%'`, so the instruction `country LIKE 'MEX%'` would return all records where the `country` starts with "MEX", then the `NOT` negates this condition to return all records where the `country` does not start with "MEX". The `%` symbol is a wildcard that matches any sequence of characters.

## Retrieve employees in Marketing

Now with the porpuse to update the computers of employees in the Marketing department in the East office, we need to retrieve all employees in the Marketing department who work in an office that starts with "East". The following query is used:

```sql
MariaDB [organization]> SELECT * 
    -> FROM employees WHERE department = 'Marketing' AND office LIKE 'East%';
+-------------+--------------+----------+------------+----------+
...
This statement differs from the previous one by change the argument of the `FROM` clause to `employees` to get the data from the employees table. Besides, the `WHERE` clause is used to filter the results based on the `department` matching "Marketing" and the `office` field starting with "East". The `LIKE` operator is used with the pattern `'East%'`, where `%` is a wildcard that matches any sequence of characters.

## Retrieve employees in Finance or Sales

To perform an update of the computers of employees in the Finance or Sales departments, we need to retrieve all employees who work in either of these departments. The following query is used:

```sql
[organization]> SELECT * FROM employees WHERE department = 'Finance' or department = 'Sales';
+-------------+--------------+----------+------------+-------------+
| employee_id | device_id    | username | department | office      |
+-------------+--------------+----------+------------+-------------+
...
This query retrieves all employees who work in either the Finance or Sales departments. The `WHERE` clause uses the `OR` operator to combine two conditions: one for the Finance department and another for the Sales department.

## Retrieve all employees not in IT

For the last is necessary to perform an update that has been made on the computers of the department IT, so we need to retrieve all employees who are not in the IT department. The following query is used:
...
This query retrieves all employees who are not in the Information Technology department. The `WHERE` clause uses the `!=` operator to filter out records where the `department` is "Information Technology". This means it will return all employees whose department is anything other than IT.

## Summary

We use the command `SELECT` to retrieve all fields from the `log_in_attempts` table, and the `employees` table. The `WHERE` clause is used to filter the results based on specific conditions using the operators `=`, `!=`, `LIKE`, and `OR` to match the desired criteria. The `LIKE` operator is particularly useful for pattern matching, allowing us to filter records based on partial matches in string fields. The `%` wildcard is used in conjunction with `LIKE` to match any sequence of characters, enabling flexible searches within text fields.